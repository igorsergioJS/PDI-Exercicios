...
int main(int argc, char** argv){
  ...
  while(1){
    cap >> image;
    image.copyTo(imgEqualize);
    //Calculo do histograma da img original
    ...
    split (imgEqualize, planes);
    //equalizar imagem capturada
    equalizeHist(planes[0], planes[0]);
    equalizeHist(planes[1], planes[1]);
    equalizeHist(planes[2], planes[2]);
    merge(planes, imgEqualize);

    cv::split (imgEqualize, planes);
    cv::calcHist(&planes[0], 1, 0, cv::Mat(), histB, 1,
                 &nbins, &histrange,
                 uniform, acummulate);
    cv::calcHist(&planes[1], 1, 0, cv::Mat(), histG, 1,
    ...
    imshow("imagemEqualizada", imgEqualize);
